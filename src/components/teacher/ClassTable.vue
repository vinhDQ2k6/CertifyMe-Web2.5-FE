...
function viewClass(classItem) {
    emit('view', classItem.classId);
    router.push({ name: 'classDetail', params: { id: classItem.classId } });
}
</script>

<template>
    <DataTable :value="classes" :loading="loading" stripedRows paginator :rows="10">
        <Column field="classCode" header="Mã lớp" sortable style="min-width: 8rem"></Column>
        ...
        <Column header="Hành động" style="min-width: 12rem">
            <template #body="slotProps">
                <Button icon="pi pi-pencil" outlined rounded class="mr-2" size="small" @click="emit('edit', slotProps.data.classId)" />
                <Button icon="pi pi-cog" outlined rounded class="mr-2" size="small" @click="emit('settings', slotProps.data.classId)" />
